...
- **MVP-3 завершён:** входящие каналы (Telegram, Google Sheets, SMS, MAX, WhatsApp,
  IP-телефония), оптимизация маршрутов, каталог товаров/`order_items` и
  расширенная аналитика (см. ниже).
...
## Аналитика за период (MVP-3)

`GET /reports/analytics?date_from=&date_to=` — расширенные KPI и разбивки:
- KPI: всего заказов, выполнено и доля выполнения (`fill_rate`), доли отказов и
  ошибок, собранная выручка, средний чек, бутыли/помпы;
- разбивки: по дням (заказы/выполнено/бутыли/выручка), по водителям
  (выполнено, бутыли, наличные/карта, ошибки/отказы), по районам;
- топ товаров по выручке (из `order_items`).

Чистый расчёт — `app/core/analytics.py`. В UI — раздел «Аналитика за период» на
экране «Отчёты».

## Каталог товаров и позиции заказа (MVP-3)

`products` (каталог) + `order_items` (позиции) добавлены дополнительным слоем:
- `GET/POST/PATCH /products` — каталог (вид `bottle_pc|bottle_pet|pump|other`, цена);
- на каждый заказ всегда формируются `order_items`: из явных `items` в `POST /orders`
  либо из количеств (`bottles_pc/pet/pumps`) по каталогу;
- если `total_amount` не задан — считается из каталожных цен; если заданы явные
  `items` — количества и сумма заказа выводятся из них;
- фиксированные поля количества остаются денормализованным кэшем (отчёты, касса,
  вместимость, баллы) и держатся в синхроне с позициями;
- миграция создаёт каталог по умолчанию (3 товара) и бэкофиллит позиции для
  существующих заказов; `GET /orders/{id}` отдаёт `items`. В UI — каталог в
  «Настройках», позиции в карточке заказа.
